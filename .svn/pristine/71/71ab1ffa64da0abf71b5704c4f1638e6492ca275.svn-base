...
void AST::mainHeader(std::vector<std::string>& profileNames) {
  ...
  AST* includeChild = new AST(token, "#include \"profile.hpp\"n");
  child.insert(subtreeItr, includeChild);
  for(std::vector<std::string>::iterator nameItr = profileNames.begin();
    ...
    AST* declareChild = new AST(token, textToAdd);
    child.insert(subtreeItr, declareChild);
  }
}
...
void AST::mainReport(std::vector<std::string>& profileNames) {
  for (std::list<AST *>::iterator unitItr = child.begin(); unitItr != child.end\
	 (); ++unitItr) {
    if ((*unitItr)->tag == "function" && (*unitItr)->getChild("name")->getName(\
									       ) == "main") {
